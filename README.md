...
- 本文は note 貼り付け用と原文 Markdown を選択でき、画像が取得済みなら本文と画像をリッチクリップボードへまとめてコピーする。表・raw HTMLなどの未対応要素があると note 用コピーを止めて対象行と手動対応を示す
...
### web-review のスクショ自動反映

`web-review/サイト名.md` の画像プレースホルダーに対して、同じサイト名の画像フォルダへ番号付きスクリーンショットを置くと、記事リポジトリへのpush時に自動で反映されます。
...
└── images/
    └── area-408-review/
        ├── 01-first-view-pc.png
        ├── 02-about.png
        └── 03-menu-sp.png
```

ファイル名は `01-...` または `01_...` の形式にし、記事内のプレースホルダー順と番号を合わせます。必要な番号が1つでも欠けている、または番号が重複している場合は、安全のため記事を変更しません。余分な番号の画像は自動では記事へ追加しません。画像の判断を「不要」または「AI生成」と明示しているプレースホルダーは自動反映の対象外です。
...
見出し、段落、改行、箇条書き、引用、コード、リンク、強調はnote貼り付け用本文へ変換します。記事内の画像を取得できている場合は、本文のテキストと画像を`text/plain`＋`text/html`のリッチクリップボードへまとめて書き込み、note側で貼り付けます。端末やブラウザがリッチクリップボードに対応しない場合は、画像用プレースホルダーを含むテキストへフォールバックします。表やraw HTMLがある場合は、対象行と手動対応を表示し、note用本文のコピーを止めます。原文Markdownへ切り替えると、変換前の本文を確認・コピーできます。

画像は記事詳細の画像準備で登録し、記事詳細を開いたときに認証付きで取得してクリップボードへ渡します。アプリはnoteの非公式APIや執筆画面への自動入力は行わないため、貼り付け結果はnote側で確認してください。リッチクリップボード非対応時は、画像の手動挿入が必要です。
